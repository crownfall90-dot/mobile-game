...
Ворота:
  ...
  G2  решение выигрывает: интервал 1.5 с при jitter 0, 1, 2; 0.8 с при 0; settle при 0.
      ...
      «Лови капли», «Стопка посуды», «Вантуз» (leak, dishes, plunger): ход — сценарий по времени из scripts,
      темп человека,
...
class Level:
    def __init__(self, path, data, index, report):
        ...
        self.timed = isinstance(data, dict) and any(isinstance(data.get(k), dict) for k in ("leak", "dishes", "plunger"))

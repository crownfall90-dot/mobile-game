...
- Прежние замыслы (Мирра, «Зелья и засовы», лаборатория) удалены по решению владельца 26.09; старые записи про 10 ремонтов и будущие акты не являются текущим заданием. [FAMILY_HOME.md](FAMILY_HOME.md) сохраняет историю концепции.
...
`--smoke` также запускает `tools/test_dialogue.gd`: уход во время реплики, выбора,
карточки, смены фона и починки. `novel_screen._exit_tree` отменяет ожидания без
выдачи сюжетного прогресса; `hub_screen._exit_tree` завершает ожидания диалога и
принадлежащего экрану Timer. Вспышка ремонта связана Tween с LocationView.
